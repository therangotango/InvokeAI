...
# https://github.com/bmaltais/kohya_ss/blob/397bf51a8cd36104e52055358e4ffd066c5858df/networks/lora_diffusers.py
...
def make_unet_conversion_map() -> Dict[str, str]:
    ...
    for i in range(3):  # num_blocks is 3 in sdxl
        ...
        if i < 3:
            ...
            sd_upsample_prefix = (  # change for sdxl
                f"output_blocks.{3*i + 2}.{2}."
            )
            unet_conversion_map_layer.append(
    ...
    for j in range(2):
        ...
        unet_conversion_map.append(
            (sd_label_embed_prefix, hf_label_embed_prefix)
        )
...
class LoRAModule(torch.nn.Module):
    ...
    def merge_to(self, multiplier=1.0):
        ...
        weight = org_weight + lora_weight.to(
...
class LoRANetwork(torch.nn.Module):
    ...
    def __init__(
        ...
        if converted:
            print(
                f"converted {converted} Stability AI's U-Net LoRA modules to"
                " Diffusers (SDXL)"
            )
        ...
        if len(skipped_te) > 0:
            print(
                f"skipped {len(skipped_te)} modules because of missing weight."
            )
        ...
        if len(skipped_un) > 0:
            print(
                f"skipped {len(skipped_un)} modules because of missing weight."
            )
    ...
    def convert_unet_modules(self, modules_dim, modules_alpha):
        ...
        assert converted_count == 0 or not_converted_count == 0, (
            f"some modules are not converted: {converted_count} converted,"
            f" {not_converted_count} not converted"
        )
    ...
    def apply_to(
        self, multiplier=1.0, apply_text_encoder=True, apply_unet=True
    ):
    ...
    def load_state_dict(
        ...
        for key in state_dict.keys():
            if state_dict[key].size() != my_state_dict[key].size():
                ...
                state_dict[key] = state_dict[key].view(
                    my_state_dict[key].size()
                )
