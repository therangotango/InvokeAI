...
def parse_args():
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "The config of the Dataset, leave as None if there's only one"
            " config."
        ),
    )
    parser.add_argument(
        ...
        help=(
            ...
            " In particular, a `metadata.jsonl` file must exist to provide the"
    ...
    parser.add_argument(
        ...
        default=1,
        help=(
            "Run fine-tuning validation every X epochs. The validation process"
            " consists of running the prompt `args.validation_prompt` multiple"
            " times: `args.num_validation_images`."
        ),
    )
    parser.add_argument(
        "--max_train_samples",
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Scale the learning rate by the number of GPUs, gradient"
    ...
    parser.add_argument(
        ...
        help=(
            ...
            " Recommended value is 5.0. More details here:"
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Whether or not to allow TF32 on Ampere GPUs. Can be used to speed"
            " up training. For more information, see"
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "The prediction_type that shall be used for training. Choose"
            " between 'epsilon' or 'v_prediction' or leave `None`. If left to"
            " `None` the default prediction type of the scheduler:"
...
def main():
    ...
    if args.report_to == "wandb":
        if not is_wandb_available():
            raise ImportError(
                "Make sure to install wandb if you want to use it for logging"
                " during training."
            )
        import wandb
        ...
        if is_xformers_available():
            ...
            if xformers_version == version.parse("0.0.16"):
                logger.warn(
                    ...
                    " xFormers to at least 0.0.17. See"
    ...
    if args.use_8bit_adam:
        try:
            ...
            raise ImportError(
                "Please install bitsandbytes to use 8-bit Adam. You can do so"
                " by running `pip install bitsandbytes`"
            )
    ...
    if args.image_column is None:
        ...
        if image_column not in column_names:
            raise ValueError(
                f"--image_column' value '{args.image_column}' needs to be one"
                f" of: {', '.join(column_names)}"
            )
    if args.caption_column is None:
        ...
        if caption_column not in column_names:
            raise ValueError(
                f"--caption_column' value '{args.caption_column}' needs to be"
                f" one of: {', '.join(column_names)}"
            )
    ...
    def tokenize_captions(examples, is_train=True):
        ...
        for caption in examples[caption_column]:
            if isinstance(caption, str):
                ...
                raise ValueError(
                    f"Caption column `{caption_column}` should contain either"
